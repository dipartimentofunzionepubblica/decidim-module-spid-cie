...
module Decidim
  module Cie
    class OmniauthCallbacksController < ::Decidim::Devise::OmniauthRegistrationsController
      ...
      rescue Decidim::Cie::Authentication::ValidationError => e
        fail_authorize(e.validation_key)
      rescue Decidim::Cie::Authentication::IdentityBoundToOtherUserError
      ...
      def create
        ...
        if invitation_token.present?
          ...
          @form = form(OmniauthCieRegistrationForm).from_params(form_params)
          @form.email ||= verified_e
          verified_e ||= form_params.dig(:email)
        end

        # Controllo che non esisti un altro account con la stessa email utilizzata con CIE
        ...
        if invited_user.present? && form_params.dig(:raw_data, :info, :email).present? && invited_user.email != form_params.dig(:raw_data, :info, :email) &&
          ...
          set_flash_message :alert, :failure, kind: "CIE", reason: t("decidim.devise.omniauth_registrations.create.email_already_exists")
        ...
        CreateOmniauthCieRegistration.call(@form, verified_e) do
          on(:ok) do |user|
            ...
            # match l'utente dell'invitation token passato come relay_state in CIE Strategy,
            # associo l'identity CIE all'utente creato nell'invitation e aggiorno l'email dell'utente con quella dello CIE.
          ...
          on(:error) do |user|
            set_flash_message :alert, :failure, kind: "CIE", reason: user.errors.full_messages.try(:first)
      ...
      rescue Decidim::Cie::Authentication::AuthorizationBoundToOtherUserError
        nil
      end

      def fail_authorize(failure_message_key = :already_authorized)
        flash[:alert] = t("failure.#{failure_message_key}", scope: "decidim.cie.omniauth_callbacks")
        redirect_to stored_location_for(resource || :user) || decidim.root_path
      end

      def omniauth_registrations_path(resource)
        decidim_cie.public_send("user_#{current_organization.enabled_omniauth_providers.dig(:cie, :tenant_name)}_omniauth_create_url")
      end
